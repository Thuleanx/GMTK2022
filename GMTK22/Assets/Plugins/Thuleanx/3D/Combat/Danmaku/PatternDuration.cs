...
namespace Thuleanx3D.Combat.Danmaku {
	public class PatternDuration : Node {
		public CompositionMode mode = CompositionMode.OVERRIDE;
		[Input(connectionType =ConnectionType.Override), Range(0f, 5f)] public float seconds;
		...
		IEnumerator<BulletInstance> _GetAllBullets() {
			...
			for (int k = 0; Subinstances.MoveNext(); k++) {
				...
				else Instance.Duration = Mathf.Max(Instance.Duration, 0) + seconds;
