...
namespace Thuleanx3D.Combat.Danmaku {
	public class PatternTilt : Node {
		public CompositionMode mode = CompositionMode.OVERRIDE;
		[Input(connectionType =ConnectionType.Override)] public float Tilt = 1;
		...
		IEnumerator<BulletInstance> _GetAllBullets() {
			...
			for (int k = 0; Subinstances.MoveNext(); k++) {
				...
				if (mode == CompositionMode.OVERRIDE)			Instance.Dir = Tilt;
				else if (mode == CompositionMode.ADDITIVE)		Instance.Dir += Tilt;
