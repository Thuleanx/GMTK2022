...
namespace Advanced.Algorithms.DataStructures
{
    ...
    public class BHeap<T> : IEnumerable<T> 
    {
        ...
        public BHeap(IEnumerable<T> initial)
            : this(initial, null) { }

        public BHeap(IComparer<T> comparer)
            : this(null, comparer) { }
        ...
        public BHeap(IEnumerable<T> initial, IComparer<T> comparer)
        {
            this.isMaxHeap = true;

            if (comparer != null)	this.comparer = comparer;
            else 					this.comparer = Comparer<T>.Default;
